...
<body>
    <main>
        ...
        <table border="1" cellpadding="0" cellspacing="0" style="margin-top: 10px;">
            ...
            <tbody>
                {{-- @dd($data['whi']) --}}
                @foreach ($data['shared'] as $key=>$shared)
                <tr @if($shared->mdr) @if($shared->mdr->score < 3.00) class="bg-warning" style="color: black;" @endif @endif>
                        <td>
                            SHARED SERVICES
                        </td>
                        <td>{{ $key+1 }}</td>
                        <td>{{ $shared->department }}</td>
                        <td class="text-center">
                            @if($shared->departments->user)
                            {{ $shared->departments->user->name }}
                            @endif
                        </td>
                        @php
                            $history = $shared->mdr_history ?? collect();
                            $m1 = $history->where('month', $month1->format('m'))
                                        ->where('year', $month1->format('Y'))
                                        ->first();
                            $m2 = $history->where('month', $month2->format('m'))
                        ...
                        <td class="text-center">
                            @if($shared->mdr)
                            {{ $shared->mdr->departments->target_date.'-'.date('M', strtotime("+ 1 month",
                            strtotime($shared->mdr->month))).'-'.$shared->mdr->year }}
                            @else
                            -
                            @endif
                        </td>
                        <td class="text-center">
                            @if($shared->mdr)
                            {{ date('d-M-Y', strtotime($shared->mdr->created_at)) }}
                            @else
                            -
                            @endif
                        </td>
                        <td class="text-center">
                            @if($shared->mdr)
                            {{ number_format($shared->mdr->timeliness,2) }}
                            @else
                            0.00
                            @endif
                        </td>
                        <td class="text-center">
                            @if($shared->mdr)
                            {{ number_format($shared->mdr->grade,2) }}
                            @else
                            0.00
                            @endif
                        </td>
                        <td class="text-center">
                            @if($shared->mdr)
                            {{ number_format($shared->mdr->innovation_scores,2) }}
                            @else
                            0.00
                            @endif
                        </td>
                        <td class="text-center">
                            @if($shared->mdr)
                            {{ number_format($shared->mdr->score,2) }}
                            @else
                            <b><i>No MDR Submitted</i></b>
                            @endif
                        </td>
                        ...
                        <td>
                            ...
                            $month = date('m', strtotime($data['year_and_month']));
                            @endphp
                            @foreach (($shared->departments->remarks)->where('year', $year)->where('month',
                            $month) as $remarks)
                            <p class="text-justify">{!! nl2br(e($remarks->remarks)) !!}</p>
                            @endforeach
                        </td>
